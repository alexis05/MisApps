...
class ServicioAPI {
  ...
  async createIfNotExistsThisAtribute({ item }) {
    const result = await this.mongoDB.existsThisAtribute(item.nombre);
    if (!result) {
      const crearItem = await this.mongoDB.create("atributos", item);
      return crearItem;
    }
    return false;
  }
