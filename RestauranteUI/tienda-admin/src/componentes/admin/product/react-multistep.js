...
const LiClass = (props) => css`
  ...
  &:before {
    ...
    background-color: ${props.state === "todo"
      ? "rgb(245, 245, 245)"
      : "#33C3F0"};
    width: 1.2em;
...
export default function MultiStep(props) {
  ...
  return (
    <div /* onKeyDown={handleKeyDown} */ className="col-12">
      <Ol className="col-12">{renderSteps()}</Ol>
      <h4 className="text-center">{props.steps[compState].label}</h4>
      <br></br>
      <hr></hr>
      <div>{props.steps[compState].component}</div>
      <div className="text-center">{renderNav(showNav)}</div>
    </div>
  );
}
